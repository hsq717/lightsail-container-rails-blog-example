...
amazon:
  ...
  bucket: <%= ENV.fetch('LIGHTSAIL_BUCKET_NAME') { 'blog-app-storage' } %>-<%= Rails.env %>
```
...
LIGHTSAIL_BUCKET_NAME=bucket-rails-joy
...
2. **"blob is nil" Error:** 
   - Run `rails active_storage:cleanup` to remove orphaned records
   - The application includes error handling to prevent crashes
   - Check `ACTIVESTORAGE_FIX.md` for detailed information

3. **S3 Connection Issues:** Verify AWS credentials and bucket permissions

4. **Database Connection Issues:** Check RDS security groups and connection parameters

5. **Image Processing Issues:** Ensure ImageMagick or libvips is installed on your system

6. **Direct Upload Not Working:**
   - Ensure JavaScript is enabled in the browser
   - Check browser console for any JavaScript errors
   - Verify S3 CORS configuration allows direct uploads

This tutorial provides a complete, production-ready Rails blog application with AWS integration and robust error handling!

## Step 17: Docker Containerization

### Docker Setup

The application includes Docker support for easy deployment and development. Two Dockerfile options are available:

1. **Dockerfile** - Basic containerization
2. **Dockerfile.optimized** - Multi-stage build with optimizations

### Building the Docker Image

**For ARM64 systems (Apple Silicon Macs):**
```bash
# Build the optimized Docker image for ARM64
docker build --platform=linux/arm64 -f Dockerfile.optimized -t blog_app:arm64 .
```

**For AMD64 systems (Intel/AMD processors):**
```bash
# Build the optimized Docker image for AMD64
docker build --platform=linux/amd64 -f Dockerfile.optimized -t blog_app:amd64 .
```

**Multi-platform build (if you have Docker Buildx):**
```bash
# Build for both platforms
docker buildx build --platform linux/amd64,linux/arm64 -f Dockerfile.optimized -t blog_app:latest .
```
...
```bash
# For ARM64 systems
docker run -p 3000:80 -e SECRET_KEY_BASE=$(openssl rand -hex 64) -e DATABASE_URL="sqlite3:storage/production.sqlite3" blog_app:arm64 ./bin/rails server -b 0.0.0.0 -p 80

# For AMD64 systems
docker run -p 3000:80 -e SECRET_KEY_BASE=$(openssl rand -hex 64) -e DATABASE_URL="sqlite3:storage/production.sqlite3" blog_app:amd64 ./bin/rails server -b 0.0.0.0 -p 80
```
...
echo 'DATABASE_URL=sqlite3:storage/production.sqlite3' >> .env

# Run with environment file
docker run -p 3000:80 --env-file .env blog_app:arm64 ./bin/rails server -b 0.0.0.0 -p 80
```
...
docker run -p 3000:80 \
  ...
  -e DATABASE_URL="postgresql://dbmasteruser:password123@ls-f56c44e93a0a3ae5a577928b197723071120cf7b.ct4q4aioa0kc.eu-west-2.rds.amazonaws.com:5432/Database-rails" \
  blog_app:arm64
```

### Docker Container Features

**Optimized Dockerfile includes:**
- ✅ Multi-stage build for smaller image size
- ✅ Ruby 3.3.5 with Alpine Linux base
...
- **Base Image:** ruby:3.3.5-alpine
- **Working Directory:** /rails
- **Exposed Port:** 80 (mapped to host port 3000)
- **User:** rails (non-root for security)
- **Environment:** Production mode by default

### Accessing the Application

Once the container is running, access the blog application at:
```
http://localhost:3000
```
...
1. **Platform Mismatch / Exec Format Error:**
   
   The "exec format error" occurs when deploying to cloud platforms that expect AMD64 architecture but you built an ARM64 image (or vice versa).
   
   **Solution for Cloud Deployment (most cloud platforms use AMD64):**
   ```bash
   # Build specifically for AMD64 (Intel/AMD processors) - most cloud platforms
   docker build --platform=linux/amd64 -f Dockerfile.optimized -t blog_app:amd64 .
   
   # Push to your container registry
   docker tag blog_app:amd64 your-registry/blog_app:latest
   docker push your-registry/blog_app:latest
   ```
   
   **For local development on Apple Silicon (ARM64):**
   ```bash
   ...
   **Multi-platform build for both architectures:**
   ...
   docker buildx build --platform linux/amd64,linux/arm64 \
     -f Dockerfile.optimized -t your-registry/blog_app:latest --push .
   ```

2. **Secret Key Missing:**
   ```bash
   # Always provide SECRET_KEY_BASE for production mode
   -e SECRET_KEY_BASE=$(openssl rand -hex 64)
   ```

3. **Database Connection Issues:**
   ```bash
   # For standalone deployment, use SQLite
...
4. **Port Conflicts:**
   ...
   docker run -p 8080:80 [other options] blog_app:amd64
   ```
...
5. **Container Won't Start:**
   ...
   docker run -it blog_app:amd64 /bin/bash
   ```

6. **Cloud Platform Deployment:**
   
   **For AWS, Google Cloud, Azure, Heroku, etc.:**
   ```bash
   # These platforms typically use AMD64 architecture
   docker build --platform=linux/amd64 -f Dockerfile.optimized -t blog_app:cloud .
   ```
   
   **For deployment platforms, use the correct run command:**
   ```bash
   # Cloud deployment typically uses AMD64 image
   docker run -p 3000:80 \
     -e SECRET_KEY_BASE=$(openssl rand -hex 64) \
     -e DATABASE_URL="sqlite3:storage/production.sqlite3" \
     blog_app:cloud ./bin/rails server -b 0.0.0.0 -p 80
   ```
...
This Docker setup provides a complete containerized solution for the Rails blog application, suitable for development, testing, and production deployment.


push the image to lightsail

aws lightsail push-container-image --region eu-west-2 --service-name container-service-lhr --label blogappnew --image blog_app:arm64
